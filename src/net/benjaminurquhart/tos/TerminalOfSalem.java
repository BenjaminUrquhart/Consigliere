...
public class TerminalOfSalem {
	...
	public static void main(String[] args) throws Exception {
		...
		PcapHandle handle = Pcaps.getDevByAddress(InetAddress.getByName("192.168.254.16")).openLive(1<<16, PromiscuousMode.NONPROMISCUOUS, 10);//Pcaps.openOffline("tos2.pcapng");
		//handle.setFilter("(((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)", BpfCompileMode.OPTIMIZE);
		IpV4Packet ipv4Packet;
		TcpPacket packet;
		Packet tmp;
		while((tmp = handle.getNextPacket()) != null) {
			...
			if(ipv4Packet.getHeader().getSrcAddr().equals(TOS_SERVER)) {
				handler.parseServerCommand(packet.getPayload().getRawData());
			}
			else if(ipv4Packet.getHeader().getDstAddr().equals(TOS_SERVER)) {
				handler.parseClientCommand(packet.getPayload().getRawData());
			}
		}
		handle.close();
	}
